...
C-----------------------------------------------------------------------
     ...
     &                     CLAY,SILT,SAND,OC
      ...
      REAL ASTCOND,AHeatCap, DampDa(NL),DampDw(NL)
...
!     NL defined in ModuleDefs.for
      ...
      REAL DampDa(NL),DampDw(NL),Omega, Del,STa(NL),STboti(NL),AMPi(NL)
...
!        DampDa(L) = 100.*SQRT(2.*STCOND(L)/(HeatCap(L)*Omega)) ! annual
!        Omega = 2.0*3.14159/(24.0*3600.0)    ! radians/s
!        DampDw(L) = 100.*SQRT(2.*STCOND(L)/(HeatCap(L)*Omega)) ! daily 
!
!       END DO       
!
