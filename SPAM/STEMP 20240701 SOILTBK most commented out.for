...
!            added by BAK 2023 11 29 for testing
     &    TA,DT,POR,                                      !Output
     &    SWREL,TcondDry, TcondSat, ASTCOND,AHeatCap,      !Output
     &    DampDa,DampDw,STBot,AMP,                         !Output
...
!     NL defined in ModuleDefs.for
      ...
      REAL DampD(NL),DampDd(NL),Omega, Del,STa(NL),STboti(NL),AMPi(NL)
      REAL TSTCond,THeatCap,ASTCond,AHeatCap,DampDa,DampDw
...
!!      END DO
!
!    Average thermal conductivity and heat capacity over whole profile
!!        TSTCond = 0.
!!        THeatCap = 0.
!!      DO L = 1, NLAYR
!!      TSTCond = TSTCond + STCond(L)*DLAYR(L)
!!            THeatCap = THeatCap + HeatCap(L)*DLAYR(L)
!!          END DO
!!      ASTCond = TSTCond/DS(NLAYR)
!!      AHeatCap = THeatCap/DS(NLAYR)
        
        
!     Calculate annual and weather pattern Damping depths (cm)
!     over whoile profile with averaged soil properties
...
!
!     Calculate the "ideal" annual curve of air temperature
!!      TA = TAV + TAMP * COS(ALX) / 2.0
!
!     Calcualte the deviation of air temperature from last 5 Days
!     average from the "ideal" annual cureve
!!      DT = ATOT / 5.0 - TA

!!       DO L = 1, NLAYR
!!         ZD    = -DSMID(L) / DampDa
!!         STi(L) = TAV + (TAMP/2.0) * COS(ALX + ZD) * EXP(ZD)
!!         Del = DT * EXP(-DSMID(L)/DampDw)
!!         ST(L) = STi(L) + Del
...
! SWREL(L) Relative soil water content between dry and saturation
