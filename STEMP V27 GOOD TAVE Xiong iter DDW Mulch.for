...
!
 ...
 ! Compute fractional area of bare soil, mulch and crop surfaces
        IF(XHLAI .GE. 3.0) THEN
            Pfrac = 1.0
        ELSE IF(XHLAI .LE. 0.0) THEN
                Pfrac = 0.0
            ELSE
            Pfrac = XHLAI/3.0
        ENDIF
        IF(MULCHCOVER .LT. 0.01) THEN
            Mfrac = 0.0
            ELSE
...
! 1.3 is the partcle density of organic matter (DeVries 1963, 1975)      
      ...
      IF(MULCHcm3pcm3 .GT. 1.0) THEN
          MULCHcm3pcm3 = 1.0
      ENDIF
      IF(MULCHcm3pcm3 .LT. 0.01) THEN
          MULCHcm3pcm3 = 0.0
          ENDIF
...
!
!   Compute annual and weather Mulch damping depths (cm)
        Omega = 2.0*3.14159/(365.0*24.0*3600.0)    ! radians/s
        DDMa = 100.*SQRT(2.*MTCond/(MHeatCap*Omega)) ! annual
        Omega = 2.0*3.14159/(5.0*24.0*3600.0)    ! radians/s
        DDMw = 100.*SQRT(2.*MTCond/(MHeatCap*Omega)) ! 5 day
...
!    and air to surface temperature differences      
!    with n-day damping depth averaged for whole profile        
      DO L = 1, NLAYR
          ZD = -DSMID(L)/DampDw
